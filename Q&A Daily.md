...
# 20161020
* 明天一定要完成对数据库表的的一些操作的公共类，将来用于导出用户的数据
* 尝试着把log4j改成绝对路径，并且提供HouseKeep的时候删除一段时间的日志和Report
...
# 20161014
   * 1. 继续完成20161012的各个指标
   * 2. check 为什么SO与Invoice条数不一样
   * 3. check lastedittime是否有问题, 13个账户是否有问题(增加或者减少账户), check金额是否正确
   * 4. 查看下面日期的记录保证所有的记录都是done的状态
...
* 4. 关于UI：【doing，可能需要到周末才能完成】
    第一个是REPORT的WEB版：所有的字段，分时间段查询，类似于DBS的查log功能【DONE】
    第二个是SQL查询的WEB 版：只列出必要字段，【DONE】
    第三个是UI也要实现能删除数据问题【DONE】
* 5. 完成尝试下把ERP ID改成Number类型【done，可以排序】，把LastestEditTime改成Datetime类型【DONE，可以排序】
* 6. HouseKeep： 1个月前的log日志需要删除，一天大概有3-400M的日志生成，一个月大概有10多G的日志生成，所以必需保证硬盘>10G【TODO】
* 7. PayTerm好像还没有修改，需要找个时间去修改【DONE,sql中添加了条件】
* 8. 检查lastestSuccess配置文件能不能生效【TODO】
* 9. 真正环境下的housekeep测试： 1.erp设为空【TODO】


# 20161011
* 1. 今天必需跑完所有的数据,查看下下13个人的checking是否正确， 【done】
* 2. 今天必需调整好所有的Report 【done】,记下来需要做Web页面
* 3. 优化log
* 4. 更新db
...
* 3. UI更改【doing，优先级Ⅱ】 --> 20161006【done】
* 4. 后天：log分类问题  --> 20161006【doing】
