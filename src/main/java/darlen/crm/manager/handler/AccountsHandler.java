...
//     1. 从ZOHO获取有效的xml
        ...
        if(StringUtils.isEmptyString(zohoStr) || zohoStr.indexOf("<error>") != -1){ //如果获取不到
            throw  new Exception("[AccountHandler],retrieveAllRowsFromZoho出现错误, retrieveZohoRecords返回Error");
        }
...
//        2. 添加
        logger.info("4.2 Begin [build2ZohoXmlSkeleton:insert], Build to ZOHO [Accounts] XML List #####################");
        List<String> addZohoXmlList = buildAdd2ZohoXml(addAccountMap,className,fieldMappingProps);
        logger.debug("End Build  addZohoXmlList. size:::"+addZohoXmlList.size());

//        3. 更新
        logger.info("4.3 Begin [build2ZohoXmlSkeleton:update], Build to ZOHO [Accounts] XML Map #####################");
