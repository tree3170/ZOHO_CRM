 ...
 * @author Darlen liu
 */
public class InvoicesHandler extends AbstractModule {
    private static InvoicesHandler handleInvoices;
    private static Logger logger =  Logger.getLogger(InvoicesHandler.class);
    private InvoicesHandler(){};

    public synchronized  static InvoicesHandler getInstance(){
        if(handleInvoices == null){
            handleInvoices = new InvoicesHandler();
...
//        System.out.println("转化ZOHO获取XML回来的Java对象:::"+response);
//
...
//            zohoModuleList = buildZohoComponentList(rows, Constants.MODULE_INVOICES_ID, Constants.ERPID);
//        }else{
//            //TODO 解析response ， 出了错
//            logger.debug("没有数据了：：：\n" + zohoStr);
...
//    }
    public List buildDBObjList(boolean isSepatateRun,String sqlWithErpIDs) throws Exception {
        logger.info("# Ⅱ：InvoicesHandler [buildDBObjList]...");
        List dbAcctList = DBUtils.getInvoiceMap(isSepatateRun,sqlWithErpIDs);
...
//    }
    ...
    public List build2ZohoObjSkeletonList(boolean isSepatateRun,String sqlWithErpIDs) throws Exception {
        logger.info("# Ⅲ: InvoicesHandler [build2ZohoObjSkeletonList]...");
        //1. 获取ZOHO对象的骨架集合
...
//    }
    ...
    public List build2ZohoXmlSkeleton(boolean isSepatateRun,String sqlWithErpIDs) throws Exception {
        logger.info("# Ⅳ: InvoicesHandler [build2ZohoXmlSkeleton]...");
        //1. 获取发送到ZOHO对象集合骨架
        ...
        //2. 添加
        logger.info("4.2 Begin [build2ZohoXmlSkeleton:insert], Build to ZOHO [Invoice] XML List #####################");
//        List<String> addZohoXmlList = buildAdd2ZohoXml(addAccountMap);
        ...
        //3. 更新
        logger.info("4.3  Begin [build2ZohoXmlSkeleton:update], Build to ZOHO [Invoice] XML Map #####################");
        ...
        //4. 删除
        logger.info("4.4 [build2ZohoXmlSkeleton:delete], Build to ZOHO [Invoice] ID List "+Constants.COMMENT_PREFIX+org.apache.commons.lang.StringUtils.join(deleteZOHOIDsList,","));
    ...
    public List execSend(boolean isSepatateRun,String sqlWithErpIDs) throws Exception {
        ...
        int addFailNum = addRecords(ModuleNameKeys.Invoices.toString(),Constants.ZOHO_CRUD_ADD,(List<String>)zohoXMLList.get(0));
        int updFailNum = updateRecords(ModuleNameKeys.Invoices.toString(),Constants.ZOHO_CRUD_UPDATE,(Map<String,String>) zohoXMLList.get(1));
        int delFailNum = delRecords(ModuleNameKeys.Invoices.toString(),Constants.ZOHO_CRUD_DELETE,(List)zohoXMLList.get(2));
...
//    }
    ...
    /**
     * 4.2 根据accountMap 组装成每100条数据的addZohoXmlList中
     * 注意：getAddRowsMap
     *
...
//        pds.add(pd2);
     ...
     * @return  zohoCompList
     */
//    public List buildZohoComponentList2(Response response, String zohoIDName, String erpIDName) {
